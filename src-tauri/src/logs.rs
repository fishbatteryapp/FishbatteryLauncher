...
fn logs_dir(app: &tauri::AppHandle) -> AppResult<PathBuf> {
  Ok(app.path().app_data_dir().map_err(into_error)?.join("data").join("logs"))
}
...
impl SessionLogger {
  ...
  fn write_line(&mut self, line: &str) -> AppResult<()> {
    ...
    self.latest.write_all(formatted.as_bytes()).map_err(into_error)?;
    self.latest.flush().map_err(into_error)?;
    self.debug.write_all(formatted.as_bytes()).map_err(into_error)?;
    self.debug.flush().map_err(into_error)?;
    self.daily_gz.write_all(formatted.as_bytes()).map_err(into_error)?;
