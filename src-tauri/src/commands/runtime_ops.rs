...
pub(crate) fn normalize_fabric_loader_version(raw: &str) -> String {
...
pub fn instances_sync_export(app: tauri::AppHandle) -> AppResult<Value> {
    ...
    let active_instance_id = db
        ...
        .filter(|id| {
            sync_instances
                .iter()
                .any(|inst| inst.get("id").and_then(|v| v.as_str()) == Some(id.as_str()))
        });
...
pub fn instances_sync_import(app: tauri::AppHandle, payload: Value) -> AppResult<Value> {
    ...
    current["updatedAt"] = json!(payload
        .get("updatedAt")
        .and_then(|v| v.as_u64())
...
fn roaming_root() -> Option<PathBuf> {
    std::env::var_os("APPDATA")
        .map(PathBuf::from)
        .or_else(|| dirs_home_dir().map(|home| home.join("AppData").join("Roaming")))
}
...
fn external_profiles_root(source: &str) -> Option<PathBuf> {
    match source {
        "curseforge" => {
            dirs_home_dir().map(|home| home.join("curseforge").join("minecraft").join("Instances"))
        }
...
fn parse_loader_from_text(raw: &str) -> (String, Option<String>) {
    ...
    if lower.contains("fabric") {
        return (
            "fabric".to_string(),
            pick_version(&["fabric-loader", "fabric loader", "fabric"]),
        );
    }
    if lower.contains("quilt") {
        return (
            "quilt".to_string(),
            pick_version(&["quilt-loader", "quilt loader", "quilt"]),
        );
    }
    if lower.contains("neoforge") || lower.contains("neo forge") {
        return (
            "neoforge".to_string(),
            pick_version(&["neoforge", "neo forge"]),
        );
    }
...
fn parse_modrinth_profile_meta(dir: &Path, fallback_name: &str) -> ExternalProfileMeta {
    ...
    ExternalProfileMeta {
        ...
        loader,
        loader_version,
        icon_path: find_existing_file_case_insensitive(
            dir,
...
async fn modrinth_get_json<T: DeserializeOwned>(
    ...
    for attempt in 0..4 {
        ...
        if response.status() == reqwest::StatusCode::TOO_MANY_REQUESTS {
            if attempt >= 3 {
                return Err(
                    "Modrinth API rate limit reached. Please retry in a moment.".to_string()
                );
            }
...
pub fn instances_update(app: tauri::AppHandle, id: String, patch: Value) -> AppResult<Value> {
    ...
    if patch.get("syncEnabled").is_some() {
        merged["syncEnabled"] = json!(patch
            .get("syncEnabled")
            .and_then(|v| v.as_bool())
            .unwrap_or(true));
    } else {
        merged["syncEnabled"] = json!(current
            .get("syncEnabled")
            .and_then(|v| v.as_bool())
            .unwrap_or(true));
    }
...
pub fn instances_export(app: tauri::AppHandle, id: String) -> AppResult<Value> {
    let safe_id = validate_id(&id)?;
    let db = read_db(&app)?;
    let inst = instance_entry(&db, &safe_id)
        .ok_or_else(|| "instances:export: instance not found".to_string())?;
    let inst_name = inst
        .get("name")
        .and_then(|v| v.as_str())
        .unwrap_or("instance");
    ...
    if lock_path.exists() {
        ...
        fs::File::open(&lock_path)
            ...
            .map_err(into_error)?;
        zip.start_file("instance.lock.json", opts)
            .map_err(into_error)?;
...
pub async fn instances_import(app: tauri::AppHandle) -> AppResult<Value> {
    ...
    if is_mrpack || has_modrinth_index {
        let imported = import_pack_archive_from_path(
            app.clone(),
            zip_path.clone(),
    ...
    let manifest_instance = manifest_value
        .get("instance")
        .cloned()
        .unwrap_or_else(|| json!({}));
...
pub async fn external_profile_import(
    ...
    if let Some(icon_path) = profile_meta.icon_path.as_ref() {
        ...
        let _ = instances_set_icon_fallback(
            ...
            created_id.clone(),
            unique_name.clone(),
            Some(if source_norm == "curseforge" {
                "orange".to_string()
            } else {
                "blue".to_string()
            }),
    ...
    let _ = loader_install(
        ...
        loader.clone(),
        if loader == "fabric" {
            cfg.get("fabricLoaderVersion")
                .and_then(|v| v.as_str())
                .map(str::to_string)
        } else if loader == "quilt" {
            cfg.get("quiltLoaderVersion")
                .and_then(|v| v.as_str())
                .map(str::to_string)
        } else if loader == "forge" {
            cfg.get("forgeVersion")
                .and_then(|v| v.as_str())
                .map(str::to_string)
        } else if loader == "neoforge" {
            cfg.get("neoforgeVersion")
                .and_then(|v| v.as_str())
                .map(str::to_string)
        } else {
            None
...
pub fn instances_import_into(app: tauri::AppHandle, instance_id: String) -> AppResult<Value> {
    ...
    let (extracted_any, imported_lockfile_raw) =
        extract_imported_instance_archive(&mut archive, &out_dir)?;
    if !extracted_any {
    ...
    if manifest_instance.get("loader").is_some() {
        patch.insert(
            ...
            manifest_instance
                .get("loader")
                .cloned()
                .unwrap_or(json!("vanilla")),
        );
        patch.insert(
            "displayLoader".to_string(),
            manifest_instance
                .get("displayLoader")
                .cloned()
                .unwrap_or(Value::Null),
...
fn local_pack_query_from_file_name(kind: &str, file_name: &str) -> String {
    ...
    let kind_label = if kind == "shaderpacks" {
        "shaderpack"
    } else {
        "resourcepack"
    };
    ...
    } else if let Some(found) = PACK_CATALOG
        .iter()
        .find(|x| x.0 == clean && x.2 == kind_label)
    {
        found.1.to_string()
    } else {
        clean
...
pub async fn local_packs_metadata(
    ...
    let project_type = if kind_norm == "shaderpacks" {
        "shader"
    } else {
        "resourcepack"
    };
    ...
    for file_name in unique {
        ...
        if let Some(hit) = payload.and_then(|v| {
            v.get("hits")
...
pub fn lockfile_generate(app: tauri::AppHandle, instance_id: String) -> AppResult<Value> {
    ...
    let mods_state: Value = read_json_file(
        &mods_state_path(&app, &safe_id)?,
        json!({ "enabled": {}, "resolved": {} }),
    );
    let packs_state: Value = read_json_file(
        &packs_state_path(&app, &safe_id)?,
        json!({ "enabled": {}, "resolved": {} }),
    );
...
pub fn servers_upsert(
    ...
    let source = entry
        ...
        .filter(|v| !v.is_empty())
        .map(str::to_string);
    let playit_tunnel_id = entry
        .get("playitTunnelId")
        .and_then(|v| v.as_str())
        .map(str::trim)
        .filter(|v| !v.is_empty())
        .map(str::to_string);
    let playit_hostname = entry
        .get("playitHostname")
        .and_then(|v| v.as_str())
        .map(str::trim)
        .filter(|v| !v.is_empty())
        .map(str::to_string);
    let playit_local_port = entry
        .get("playitLocalPort")
        .and_then(|v| match v {
    ...
    if let Some(id) = incoming_id {
            ...
            .find(|x| x.get("id").and_then(|v| v.as_str()) == Some(id.as_str()))
        {
            ...
            existing["source"] = source.clone().map(Value::String).unwrap_or(Value::Null);
            existing["playitTunnelId"] = playit_tunnel_id
                .clone()
                .map(Value::String)
                .unwrap_or(Value::Null);
            existing["playitHostname"] = playit_hostname
                .clone()
                .map(Value::String)
                .unwrap_or(Value::Null);
    ...
    if out.is_null() {
        ...
        let created = json!({
          ...
          "source": source,
          "playitTunnelId": playit_tunnel_id,
...
pub fn servers_export_profile(
    ...
    let server = servers_state
        ...
        .and_then(|rows| {
            rows.iter()
                .find(|x| x.get("id").and_then(|v| v.as_str()) == Some(server_id.as_str()))
        })
        .cloned()
    ...
    let profile = json!({
      ...
      "server": {
        ...
        "source": server.get("source").cloned().unwrap_or(Value::Null),
    ...
    zip.start_file("server-profile.json", opts)
        .map_err(into_error)?;
    zip.write_all(
    ...
    if cfg_dir.exists() {
        ...
        for file in cfg_files {
            let rel = file
                ...
                .to_string_lossy()
                .replace('\\', "/");
            zip.start_file(format!("config/{rel}"), opts)
                .map_err(into_error)?;
            let mut bytes = Vec::new();
            fs::File::open(&file)
                .map_err(into_error)?
...
pub async fn loader_pick_version(loader: String, mc_version: String) -> AppResult<Option<String>> {
    ...
    if loader_norm == "quilt" {
        ...
        let fallback = rows
            .iter()
            .find_map(|row| row.loader.as_ref()?.version.clone());
    ...
    Err(format!(
        "loader:pickVersion: unsupported loader {loader_norm}"
    ))
}
    ...
    if (loader_norm == "forge" || loader_norm == "neoforge") && resolved.is_some() {
        ...
        for candidate in version_candidates {
          ...
          format!("neoforge-{candidate}-installer.jar"),
            ...
            let already_ok =
                out_file.exists() && fs::metadata(&out_file).map_err(into_error)?.len() > 0;
            if already_ok {
...
pub fn mods_list(app: tauri::AppHandle, instance_id: String) -> AppResult<Value> {
    ...
    for id in keys {
        ...
        let status = resolved
            ...
            .to_string();
        let display_name = cat.map(|x| x.1.to_string()).unwrap_or_else(|| {
            id.split('-')
                .filter(|p| !p.is_empty())
                .map(|p| {
    ...
    for v in versions {
        ...
        if let Some(loader_id) = loader {
            ...
            let loader_match = if loader_id == "quilt" {
                ...
                loaders
                    .iter()
                    .any(|x| matches!(x.as_str(), Some("quilt" | "fabric")))
            } else {
...
async fn install_required_project_dependency(
    ...
    let Some(r) = resolve_latest_modrinth(client, project_id, mc_version, loader_filter).await?
    else {
        return Ok(Vec::new());
    };
...
async fn mods_refresh_internal(
    ...
    for (id, _name, required, project_id) in MOD_CATALOG {
        ...
        let Some(r) =
            resolve_latest_modrinth(&client, project_id, &mc_version, loader_filter).await?
        else {
            clear_prefixed_files(&mods_dir, id, ".jar")?;
            resolved_obj.insert(
                id.to_string(),
    ...
    if target_enabled && !found_managed_file {
        ...
        if !upstream.is_empty() {
            ...
            let cache_name = format!(
                "{}-{}.jar",
                id,
                if sha1.is_empty() { &upstream } else { sha1 }
            );
            let cache_path = cache_dir.join(cache_name);
            if cache_path.exists() && fs::metadata(&cache_path).map_err(into_error)?.len() > 0 {
                clear_prefixed_files(&mods_dir, &id, ".jar")?;
                ...
                if let Some(resolved_obj) =
                    state.get_mut("resolved").and_then(|v| v.as_object_mut())
                {
                    if let Some(entry) = resolved_obj.get_mut(&id).and_then(|v| v.as_object_mut()) {
...
#[command]
pub async fn mods_plan_refresh(
    app: tauri::AppHandle,
    instance_id: String,
    mc_version: Option<String>,
) -> AppResult<Value> {
...
pub async fn mods_sync_bridge(
    ...
    let tag = std::env::var("FISHBATTERY_CAPE_BRIDGE_TAG")
        .unwrap_or_default()
        .trim()
        .to_string();
    ...
    let desired = assets
        ...
        .find(|a| {
            let name = a
                ...
                .to_ascii_lowercase();
            name.ends_with(".jar")
                && name.contains(&mc.to_ascii_lowercase())
                && name.contains(loader_hint)
        })
        .or_else(|| {
            assets.iter().find(|a| {
                let name = a
        ...
        .ok_or_else(|| {
            "mods:syncBridge: No suitable bridge JAR found in release assets".to_string()
        })?;
    ...
    for entry in fs::read_dir(&mods_dir).map_err(into_error)? {
        ...
        let is_bridge = lower.contains("cape-bridge")
            || (lower.contains("fishbattery") && lower.contains("bridge"));
...
pub fn packs_set_enabled(
    ...
    let dir = instance_dir(&app, &safe_id)?.join(if kind == "shaderpack" {
        "shaderpacks"
    } else {
        "resourcepacks"
    });
...
pub async fn packs_refresh(
    ...
    let mut resolved_obj = state
        ...
        .cloned()
        .unwrap_or_default();
    let cache_dir = app_data_root(&app)?
        .join("data")
        .join("cache")
        .join("packs");
    fs::create_dir_all(&cache_dir).map_err(into_error)?;
        ...
        if let Some(exe_dir) = exe.parent() {
            let candidates = [
                ...
                exe_dir
                    ...
                    .join("secrets")
                    .join("curseforge-api-key.txt"),
                exe_dir.join("resources").join("curseforge-api-key.txt"),
            ];
            for p in candidates {
                if let Some(k) = read_secret_text(&p) {
                    return Some(k);
...
fn safe_modrinth_content_file_name(
    kind: &str,
    project_id: &str,
...
pub async fn modrinth_mods_install(
    ...
    let version: Value = if let Some(v_id) = version_id {
        modrinth_get_json(
            ...
            &format!(
                "https://api.modrinth.com/v2/version/{}",
                urlencoding::encode(v_id.trim())
            ),
        )
        .await?
    } else {
        let versions_url =
            modrinth_versions_endpoint(pid, Some(mc.as_str()), Some(loader.as_str()))?;
        let versions: Vec<Value> = modrinth_get_json(&client, &versions_url).await?;
        ...
        for v in versions {
            let game_versions = v
                ...
                .cloned()
                .unwrap_or_default();
            let has_mc = game_versions
                .iter()
                .any(|x| x.as_str() == Some(mc.as_str()));
            if !has_mc {
                continue;
            }
            if loader != "vanilla" {
...
pub async fn modrinth_content_install(
    ...
    let version: Value = if let Some(v_id) = version_id {
        ...
        picked
            .ok_or_else(|| format!("No compatible Modrinth {kind_label} version found for {mc}"))?
    };
    ...
    let cache_kind = if kind_label == "shaderpack" {
        "shaderpacks"
    } else {
        "resourcepacks"
    };
...
pub async fn fabric_install(
    ...
    let mc = mc_version.trim().to_string();
    let lv = normalize_fabric_loader_version(&loader_version);
    if mc.is_empty() {
    ...
    if p == "technic" {
      ...
      "providerPacksInstall: missing CurseForge API key. Set FISHBATTERY_CURSEFORGE_API_KEY or create secrets/curseforge-api-key.txt".to_string()
        ...
        let pack_bytes = client
            ...
            .map_err(into_error)?;
        let mut archive =
            ZipArchive::new(std::io::Cursor::new(pack_bytes.to_vec())).map_err(into_error)?;
        let out_root = instance_dir(&app, &created_id)?;
        let mut installed_manifest_mods = 0usize;
        let mut content_meta =
            read_content_metadata(&app, &created_id).unwrap_or_else(|_| json!({}));
        ...
        if let Some(mf) = manifest.as_ref() {
            ...
            if effective_loader != created_loader || effective_mc != created_mc {
                ...
                if effective_loader != "vanilla" {
                    let resolved =
                        loader_pick_version(effective_loader.clone(), effective_mc.clone()).await?;
        ...
        if let Some(mf) = manifest {
            if let Some(files) = mf.get("files").and_then(|v| v.as_array()) {
                ...
                for item in files {
                    ...
                    let dep_url =
                        if let Some(url) = dep_data.get("downloadUrl").and_then(|v| v.as_str()) {
                            url.to_string()
                        } else {
        ...
        notes.push(format!(
            "Resolved {installed_manifest_mods} required manifest mod files."
        ));
        return Ok(json!({
          "instance": created,
          "notes": notes
        ...
        if let Ok(pack_id_num) = numeric.parse::<u64>() {
            let detail: Value = reqwest::Client::new()
                .get(format!(
                    "https://api.modpacks.ch/public/modpack/{pack_id_num}"
                ))
                .header("user-agent", MODRINTH_USER_AGENT)
                .send()
                .await
            ...
            if let Some(v) = detail
                ...
                .and_then(|x| x.as_array())
            {
                if let Some(modloader) = v
                    .iter()
                    .find(|t| t.get("type").and_then(|x| x.as_str()) == Some("modloader"))
                {
            ...
            if let Some(v_id) = version_id {
                let version_detail: Value = reqwest::Client::new()
                    .get(format!(
                        "https://api.modpacks.ch/public/modpack/{pack_id_num}/{v_id}"
                    ))
                    .header("user-agent", MODRINTH_USER_AGENT)
                    .send()
                    .await
                ...
                if let Some(files) = version_detail.get("files").and_then(|x| x.as_array()) {
                    let root = created
                        ...
                        .transpose()?
                        .ok_or_else(|| {
                            "providerPacksInstall: missing created instance id".to_string()
                        })?;
                    let client = reqwest::Client::new();
                    let mut downloaded = 0usize;
                    for f in files {
                    ...
                    notes.push(format!(
                        "Downloaded {downloaded} required files from FTB metadata."
                    ));
                }
            }
        ...
        notes.push(format!(
            "Created instance from ATLauncher metadata ({pid})."
        ));
        notes.push(
            "Direct ATLauncher file sync is not available from current public API.".to_string(),
        );
    }
