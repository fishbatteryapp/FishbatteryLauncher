...
fn position_for_drag(window: &Window, cursor_x: i32, cursor_y: i32, anchor_ratio: f64) -> AppResult<Position> {
  let size = window.outer_size().map_err(into_error)?;
  let width = i32::try_from(size.width).unwrap_or(i32::MAX);
  let next_x = (f64::from(cursor_x) - f64::from(width) * clamp_anchor_ratio(anchor_ratio)).round() as i32;
...
#[command]
pub fn window_drag_restore(window: Window, cursor_x: i32, cursor_y: i32, anchor_ratio: f64) -> AppResult<bool> {
  if !window.is_maximized().map_err(into_error)? {
    return Ok(false);
  }
...
#[command]
pub fn window_drag_move(window: Window, cursor_x: i32, cursor_y: i32, anchor_ratio: f64) -> AppResult<bool> {
  if window.is_maximized().map_err(into_error)? {
    return Ok(false);
  }
  let next_pos = position_for_drag(&window, cursor_x, cursor_y, anchor_ratio)?;
...
#[command]
pub fn window_set_title_bar_theme(window: Window, color: String, symbol_color: String) -> AppResult<bool> {
