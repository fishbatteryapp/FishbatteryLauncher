...
fn find_instance_mut<'a>(db: &'a mut Value, instance_id: &str) -> Option<&'a mut Value> {
  db.get_mut("instances")
    ...
    .and_then(|items| items.iter_mut().find(|x| x.get("id").and_then(|v| v.as_str()) == Some(instance_id)))
}
...
fn java_bin_candidates_for_root(root: &Path, channel: &str) -> Vec<PathBuf> {
  if cfg!(target_os = "windows") {
    vec![
      root.join("runtime").join(channel).join("bin").join("javaw.exe"),
      root.join("runtime").join(channel).join("bin").join("java.exe"),
    ]
  } else {
...
fn parse_java_major_from_text(text: &str) -> Option<u32> {
  ...
  first.split('.').next().unwrap_or("").trim().parse::<u32>().ok()
}
...
fn resolve_java_executable(app: &tauri::AppHandle, mc_version: Option<&str>) -> (String, bool, String) {
...
fn account_exists(app: &tauri::AppHandle, account_id: &str) -> bool {
  ...
  db.get("accounts")
    .and_then(|v| v.as_array())
    .map(|items| items.iter().any(|x| x.get("id").and_then(|v| v.as_str()) == Some(account_id)))
    .unwrap_or(false)
}
...
fn ensure_library_download_fields(lib: &mut Value) {
  ...
  if lib.get("downloads").is_none() || !lib.get("downloads").map(|v| v.is_object()).unwrap_or(false) {
    lib["downloads"] = json!({});
  }
...
fn rule_matches(rule: &Value, features: &HashMap<&str, bool>) -> bool {
  if let Some(os) = rule.get("os").and_then(|v| v.as_object()) {
    ...
    if let Some(arch) = os.get("arch").and_then(|v| v.as_str()) {
      let cur = if cfg!(target_pointer_width = "64") { "x86_64" } else { "x86" };
...
async fn download_file_if_missing(client: &reqwest::Client, url: &str, target: &Path) -> AppResult<()> {
...
fn sanitize_legacy_jvm_flags_for_java(args: Vec<String>, java_major: Option<u32>) -> (Vec<String>, Vec<String>) {
...
fn update_instance_loader_field(app: &tauri::AppHandle, instance_id: &str, field: &str, value: &str) -> AppResult<()> {
...
async fn install_quilt_profile(app: tauri::AppHandle, mc_version: &str, loader_version: &str) -> AppResult<String> {
  ...
  fs::write(out_json, serde_json::to_string_pretty(&final_profile).map_err(into_error)?).map_err(into_error)?;

  let vanilla_jar = versions_root(&app)?.join(mc_version).join(format!("{mc_version}.jar"));
  let out_jar = vdir.join(format!("{quilt_id}.jar"));
  if vanilla_jar.exists() && (!out_jar.exists() || fs::metadata(&out_jar).map_err(into_error)?.len() == 0) {
...
async fn resolve_launch_profile_id(
  ...
  if loader == "fabric" {
      ...
      .filter(|s| !s.is_empty())
    {
      ...
      let resolved = runtime_ops::loader_pick_version("fabric".to_string(), mc_version.clone())
        .await?
    ...
    .await?;
    return Ok((format!("fabric-loader-{loader_version}-{mc_version}"), loader));
  }
  ...
  if loader == "quilt" {
      ...
      .filter(|s| !s.is_empty())
    {
      ...
      let resolved = runtime_ops::loader_pick_version("quilt".to_string(), mc_version.clone())
        .await?
  ...
  if loader == "forge" || loader == "neoforge" {
    ...
    if let Some(found) = find_forge_like_profile_id(&versions, &mc_version, &loader, &loader_version) {
      let _ = emit_launch_log_app(&app, format!("[launcher] Reusing existing {loader} profile: {found}"));
    ...
    for args in installer_attempts {
      ...
      if out.status.success() {
        ...
        let _ = emit_launch_log_app(
          &app,
          format!("[launcher] {loader} installer completed without profile: {}", args.join(" ")),
        );
        continue;
      }
    ...
    if !install_ok {
      ...
      let _ = emit_launch_log_app(&app, format!("[launcher] {loader} installer failed: {msg}"));
...
pub async fn launch(
  ...
  let inst = find_instance(&db, &safe_instance_id).ok_or_else(|| "launch: instance not found".to_string())?;
  ...
  let _ = emit_launch_log(
    &window,
    format!("[launcher] Starting launch for instance {}", safe_instance_id),
  );
  ...
  let bridge = runtime_ops::mods_sync_bridge(app.clone(), safe_instance_id.clone(), mc_version_opt.clone()).await;
  match bridge {
    Ok(result) => {
      if result.get("installed").and_then(|v| v.as_bool()).unwrap_or(false) {
        let asset = result.get("assetName").and_then(|v| v.as_str()).unwrap_or("bridge asset");
    ...
    Err(err) => {
      ...
      if supported && loader == "fabric" {
        let _ = emit_launch_log(&window, format!("[launcher] Bridge sync failed (required): {err}"));
  ...
  let requested_memory_mb = inst.get("memoryMb").and_then(|v| v.as_u64()).unwrap_or(4096);
  ...
  if bundled {
    let _ = emit_launch_log(&window, format!("[launcher] Using bundled Java {java_label}: {java_exe}"));
  } else {
  ...
  match Command::new(&java_exe).arg("-version").output() {
    ...
    Err(err) => {
      return Ok(json!({ "ok": false, "error": format!("launch: failed to run java ({err})") }));
    }
  }
  ...
  let (profile_id, loader_kind) =
    match resolve_launch_profile_id(app.clone(), &java_exe, &safe_instance_id, inst).await {
      Ok(v) => v,
      Err(err) => {
        let _ = emit_launch_log(&window, format!("[launcher] Launch preparation failed: {err}"));
  ...
  let _ = emit_launch_log(
    &window,
    format!("[launcher] Resolved profile: {profile_id} (loader={loader_kind}, mc={mc_version})"),
  );
  ...
  if let Some(cmd) = pre_launch.as_deref() {
    if let Err(err) = run_hook_command("pre-launch", cmd, &app) {
      ...
      return Ok(json!({ "ok": false, "error": format!("launch: pre-launch hook failed: {err}") }));
    }
  }
  ...
  let mut profile = match load_profile_recursive(&versions_dir, &profile_id, 0) {
    Ok(p) => p,
    Err(err) => {
      let _ = emit_launch_log(&window, format!("[launcher] Failed to load merged profile: {err}"));
      return Ok(json!({ "ok": false, "error": err }));
    }
  };
  let client = reqwest::Client::builder()
    ...
    for lib in libs.iter_mut() {
      ...
      if let (Some(path), Some(url)) = (
        ...
        if let Err(err) = download_file_if_missing(&client, url, &target).await {
          ...
          let msg = format!("launch: failed downloading library {name} from {url}: {err}");
          let _ = emit_launch_log(&window, format!("[launcher] {msg}"));
          return Ok(json!({ "ok": false, "error": msg }));
        }
      ...
      if let Some(classifier_key) = classifier_key_for_os(lib) {
        if let (Some(path), Some(url)) = (
          ...
          if let Err(err) = download_file_if_missing(&client, url, &native_jar).await {
            let msg = format!("launch: failed downloading native library from {url}: {err}");
            let _ = emit_launch_log(&window, format!("[launcher] {msg}"));
            return Ok(json!({ "ok": false, "error": msg }));
          }
          if let Err(err) = extract_native_jar(&native_jar, &natives_dir) {
            let msg = format!("launch: failed extracting native jar {}: {}", native_jar.to_string_lossy(), err);
            let _ = emit_launch_log(&window, format!("[launcher] {msg}"));
            return Ok(json!({ "ok": false, "error": msg }));
          }
        }
  ...
  if !client_jar.exists() || fs::metadata(&client_jar).map_err(into_error)?.len() == 0 {
    ...
    if let Err(err) = download_file_if_missing(&client, client_url, &client_jar).await {
      let _ = emit_launch_log(&window, format!("[launcher] Failed downloading client jar: {err}"));
      return Ok(json!({ "ok": false, "error": format!("launch: failed downloading client jar ({err})") }));
    }
  }
  ...
  let asset_index_id = profile
    ...
    .unwrap_or("legacy")
    .to_string();
  let asset_index_path = assets_dir.join("indexes").join(format!("{asset_index_id}.json"));
  ...
  if !asset_index_path.exists() || fs::metadata(&asset_index_path).map_err(into_error)?.len() == 0 {
      ...
      .and_then(|v| v.as_str())
    {
      if let Err(err) = download_file_if_missing(&client, url, &asset_index_path).await {
        let msg = format!("launch: failed downloading asset index {asset_index_id} from {url}: {err}");
        let _ = emit_launch_log(&window, format!("[launcher] {msg}"));
        return Ok(json!({ "ok": false, "error": msg }));
      }
    }
    ...
    if let Ok(idx_json) = serde_json::from_str::<Value>(&raw) {
      if let Some(objects) = idx_json.get("objects").and_then(|v| v.as_object()) {
        for (_, obj) in objects {
          let hash = obj.get("hash").and_then(|v| v.as_str()).unwrap_or("").trim();
          if hash.len() < 2 {
            continue;
          }
          let sub = &hash[..2];
  ...
  let auth = match read_account_mclc_auth(&app, &safe_account_id) {
    Ok(a) => a,
    Err(err) => {
      let _ = emit_launch_log(&window, format!("[launcher] Account auth unavailable: {err}"));
      return Ok(json!({ "ok": false, "error": err }));
    }
  };
  ...
  let memory_mb = inst.get("memoryMb").and_then(|v| v.as_u64()).unwrap_or(4096);
  let java_is_32bit = is_likely_32bit_java(&java_exe);
  let effective_memory_mb = if java_is_32bit { memory_mb.min(1024) } else { memory_mb };
    ...
    if sig_path.exists() {
      if let Ok(sig_raw) = fs::read_to_string(&sig_path) {
        ...
        if !sig.is_empty() {
          ...
          let _ = emit_launch_log(&window, "[capes] Attached launcher cape signature".to_string());
        }
      }
    }
  } else {
    let _ = emit_launch_log(&window, "[capes] No local launcher cape selected for this account".to_string());
  }
  ...
  if !has_classpath_flag {
    resolved_jvm_args.push(format!("-Djava.library.path={}", natives_dir.to_string_lossy()));
  ...
  thread::spawn(move || {
    ...
    if let Some(cmd) = post_exit.as_deref() {
      if let Err(err) = run_hook_command("post-exit", cmd, &app_wait) {
        let _ = emit_launch_log_app(&app_wait, format!("[hook] post-exit hook failed: {err}"));
      }
    }
...
#[command]
pub async fn launch_apply_fix(app: tauri::AppHandle, instance_id: String, action: String) -> AppResult<Value> {
  ...
  let db = read_instances_db(&app)?;
  let inst = find_instance(&db, &safe_instance_id).ok_or_else(|| "Instance not found".to_string())?;
  let mc_version = inst
    .get("mcVersion")
    .and_then(|v| v.as_str())
  ...
  if action_norm == "refresh-mods" {
    if loader != "fabric" {
      return Err("Mod refresh is currently implemented for Fabric instances only".to_string());
    }
    let _ = runtime_ops::mods_refresh(app.clone(), safe_instance_id.clone(), Some(mc_version.clone())).await?;
    return Ok(json!({
      "ok": true,
      "action": action_norm,
...
pub fn rollback_create_snapshot(
  ...
  let mods_state: Value = read_json_file(&mods_state_path(&app, &safe_instance_id)?, json!({ "enabled": {} }));
  let packs_state: Value = read_json_file(&packs_state_path(&app, &safe_instance_id)?, json!({ "enabled": {} }));
  ...
  if !rollback.get("snapshots").map(|v| v.is_array()).unwrap_or(false) {
...
#[command]
pub async fn rollback_restore_latest(app: tauri::AppHandle, instance_id: String) -> AppResult<Value> {
  ...
  let mut db = read_instances_db(&app)?;
  let inst = find_instance_mut(&mut db, &safe_instance_id).ok_or_else(|| "Instance not found".to_string())?;
  if let Some(snapshot_instance) = latest.get("instance").and_then(|v| v.as_object()) {
    ...
    assign(&mut inst["fabricLoaderVersion"], snapshot_instance.get("fabricLoaderVersion"));
    assign(&mut inst["quiltLoaderVersion"], snapshot_instance.get("quiltLoaderVersion"));
    assign(&mut inst["forgeVersion"], snapshot_instance.get("forgeVersion"));
    assign(&mut inst["neoforgeVersion"], snapshot_instance.get("neoforgeVersion"));
    assign(&mut inst["memoryMb"], snapshot_instance.get("memoryMb"));
    assign(&mut inst["jvmArgsOverride"], snapshot_instance.get("jvmArgsOverride"));
    assign(&mut inst["instancePreset"], snapshot_instance.get("instancePreset"));
  }
  db["updatedAt"] = json!(now_ms());
  write_instances_db(&app, &db)?;

  let mut mods_state: Value = read_json_file(&mods_state_path(&app, &safe_instance_id)?, json!({ "enabled": {}, "resolved": {} }));
  mods_state["enabled"] = latest.get("modsEnabled").cloned().unwrap_or_else(|| json!({}));
  write_json_file(&mods_state_path(&app, &safe_instance_id)?, &mods_state)?;

  let mut packs_state: Value = read_json_file(&packs_state_path(&app, &safe_instance_id)?, json!({ "enabled": {}, "resolved": {} }));
  packs_state["enabled"] = latest.get("packsEnabled").cloned().unwrap_or_else(|| json!({}));
  ...
  if !mc_version.trim().is_empty() {
    let _ = runtime_ops::packs_refresh(app.clone(), safe_instance_id.clone(), Some(mc_version)).await?;
  }
