...
fn updater_channel_path(app: &tauri::AppHandle) -> AppResult<PathBuf> {
  Ok(app_data_root(app)?.join("data").join("updater-channel.json"))
}
...
fn write_updater_channel(app: &tauri::AppHandle, channel: &str) -> AppResult<()> {
  ...
  fs::write(path, serde_json::to_string_pretty(&json!({ "channel": channel })).map_err(into_error)?)
    .map_err(into_error)?;
  Ok(())
}
...
fn store_pending_update(app: &tauri::AppHandle, update: &tauri_plugin_updater::Update) -> AppResult<()> {
  let state = app.state::<AppState>();
  let mut pending = state
    .updater_pending_update
    .lock()
...
fn build_updater(app: &tauri::AppHandle, channel: &str) -> AppResult<tauri_plugin_updater::Updater> {
  let Some(pubkey) = updater_pubkey_from_env() else {
    return Err(
      "updater: missing pubkey, set FISHBATTERY_UPDATER_PUBKEY (or TAURI_UPDATER_PUBKEY)".to_string(),
    );
  };
...
pub fn updater_get_state(app: tauri::AppHandle) -> AppResult<Value> {
  ...
  Ok(guard.clone().unwrap_or_else(|| updater_state_default(&app, "Updates not checked yet.")))
}
...
pub async fn updater_check(app: tauri::AppHandle) -> AppResult<bool> {
  ...
  let _ = updater_set_status(&app, "checking", &format!("Checking for updates ({channel} channel)..."))?;
...
fn hardware_summary() -> Value {
  ...
  let cpu_cores = std::thread::available_parallelism()
    .map(|v| v.get() as u64)
    .unwrap_or(1);
  let cpu_model = std::env::var("PROCESSOR_IDENTIFIER").unwrap_or_else(|_| "Unknown CPU".to_string());
  json!({
...
fn build_optimizer_preview(profile: &str) -> Value {
  ...
  let total_ram_mb = hw.get("totalRamMb").and_then(|v| v.as_u64()).unwrap_or(8192);
  ...
  let (gc, jvm_args, mods_to_enable): (&str, &str, Vec<&str>) = if prof == "aggressive" {
    ...
    (
      ...
      PERF_MOD_IDS.iter().copied().filter(|x| *x != "c2me").collect(),
    )
  } else {
    (
      "G1GC",
...
#[command]
pub async fn optimizer_apply(app: tauri::AppHandle, instance_id: String, profile: String) -> AppResult<Value> {
  ...
  for mod_id in PERF_MOD_IDS {
    ...
    let _ = runtime_ops::mods_set_enabled(app.clone(), instance_id.clone(), (*mod_id).to_string(), should_enable);
  }
  let mc_version = inst
    .get("mcVersion")
    .and_then(|v| v.as_str())
...
fn benchmark_path(app: &tauri::AppHandle, instance_id: &str) -> AppResult<PathBuf> {
  Ok(instances_root(app)?.join(instance_id).join("benchmark-results.json"))
}
...
#[command]
pub fn benchmark_run(app: tauri::AppHandle, instance_id: String, profile: Option<String>) -> AppResult<Value> {
  ...
  let inst = db
    ...
    .ok_or_else(|| "Instance not found".to_string())?;
  let hw = hardware_summary();
  let mem = inst.get("memoryMb").and_then(|v| v.as_u64()).unwrap_or(4096);
...
pub fn diagnostics_export(app: tauri::AppHandle, _window: Window) -> AppResult<Value> {
  ...
  zip
    ...
    .map_err(into_error)?;
  zip
    .write_all(serde_json::to_string_pretty(&meta).map_err(into_error)?.as_bytes())
    .map_err(into_error)?;
