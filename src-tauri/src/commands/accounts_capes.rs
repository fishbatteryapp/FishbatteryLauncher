    ...
    while Instant::now() < timeout {
        ...
        if let Ok(err) = serde_json::from_str::<MicrosoftDeviceErrorResponse>(&text) {
            match err.error.as_deref().unwrap_or_default() {
                ...
                "expired_token" => {
                    return Err(
                        "Microsoft sign-in timed out. Retry and finish the browser sign-in sooner."
                            .to_string(),
                    );
                }
                _ => {
...
async fn authenticate_xbox_live(
    ...
    let user_hash = payload
        ...
        .filter(|s| !s.is_empty())
        .ok_or_else(|| {
            "Xbox Live authentication succeeded but no user hash was returned.".to_string()
        })?;
...
async fn verify_minecraft_ownership(
    client: &reqwest::Client,
    minecraft_access_token: &str,
) -> AppResult<()> {
    let res = client
    ...
    if payload.items.is_empty() {
        return Err(
            "Microsoft sign-in succeeded, but this account does not appear to own Minecraft Java."
                .to_string(),
        );
    }
    Ok(())
...
async fn fetch_avatar_to_cache(path: &Path, id: &str, username: &str) -> AppResult<()> {
    ...
    Err(format!(
        "accounts_get_avatar: avatar fetch failed ({last_error})"
    ))
}
...
fn merge_cape_payloads(
    primary: &serde_json::Value,
    secondary: &serde_json::Value,
) -> serde_json::Value {
    let mut out: Vec<serde_json::Value> = Vec::new();
    let mut seen: std::collections::HashSet<String> = std::collections::HashSet::new();
    for row in extract_cape_items(primary)
        .into_iter()
        .chain(extract_cape_items(secondary).into_iter())
    {
...
async fn payload_to_local_cape_catalog(
    app: &tauri::AppHandle,
    ...
    for raw in raw_items {
        ...
        let preview_inline = pick_first_string(&[
            raw.get("previewDataUrl")
                .and_then(|v| v.as_str())
                .map(str::to_string),
            raw.get("thumbnailDataUrl")
                .and_then(|v| v.as_str())
                .map(str::to_string),
        ]);
...
async fn fetch_remote_selected_cape_id(app: &tauri::AppHandle) -> AppResult<Option<String>> {
    ...
    let access_token = session.access_token.unwrap_or_default().trim().to_string();
...
async fn persist_remote_selected_cape_id(
    app: &tauri::AppHandle,
    cape_id: Option<&str>,
...
fn account_minecraft_token(account: &StoredAccount) -> AppResult<String> {
    ...
    if token.is_empty() {
        return Err(
            "Minecraft access token is missing for this account. Remove and re-add the account."
                .to_string(),
        );
    }
    Ok(token)
...
fn bytes_to_data_url(bytes: &[u8], content_type: Option<&str>) -> String {
    let mime = content_type
        ...
        .trim()
        .to_ascii_lowercase();
    let safe_mime = if mime.starts_with("image/") {
        mime
    } else {
        "image/png".to_string()
    };
...
pub async fn capes_set_official_active(
    ...
    let response = if let Some(raw) = cape_id {
        ...
        let mut res = client
            ...
            .map_err(into_error)?;
        if !res.status().is_success()
            && (res.status().as_u16() == 404 || res.status().as_u16() == 405)
        {
            res = client
                .post(endpoint)
                .header(reqwest::header::AUTHORIZATION, format!("Bearer {token}"))
...
pub async fn skins_upload_official(
    ...
    let token = account_minecraft_token(&account)?;
    let client = reqwest::Client::new();
    let chosen_variant = match variant
